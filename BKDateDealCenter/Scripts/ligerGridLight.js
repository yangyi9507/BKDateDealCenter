...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 20, 50, 100, 500, 1000],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
        ...
        mouseoverRowCssClass: '', //l-grid-row-over 优化
    ...
    $.ligerDefaults.Grid.editors['select'] =
     $.ligerDefaults.Grid.editors['combobox'] =
     {
         create: function (container, editParm) {
             ...
             input.focus(); //---wangfeng 2014-1-25 ---
             return input;
         },
         getValue: function (input, editParm) {
             return input.liger('option', 'value');
    ...
    $.ligerDefaults.Grid.editors['string'] =
     $.ligerDefaults.Grid.editors['text'] = {
         create: function (container, editParm) {
             var input = $("<input type='text' class='l-text-editing' onfocus='this.select()' />");
             container.append(input);
             input.ligerTextBox();
             input.focus(); // wangfeng 2013-12-10
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm) {
            ...
            g.changedCells = {}; //---update bug wangfeng 2013.8.8 刷新后清空数据----
            var clause = null;
            var loadServer = true;
        ...
        */
        ...
        changePage: function (ctype) {
            var g = this, p = this.options;
            if (g.loading) return true;
            if (p.dataAction != "local" && g.isDataChanged && false)//!confirm(p.isContinueByDataChanged) --wangfeng 20130902 去掉弹出提示
                return false;
        ...
        _showData: function () {
            ...
            if (p.usePager) {
                ...
                //---update bug wangfeng 2013.12.20 新加入强制判断页码
                if (p.newPage > p.pageCount) {
                    p.newPage = p.pageCount;
                    p.page = p.pageCount;

                    g.loadData(p.where); //重新进行查询
                    return;
                } // end wangfeng
            ...
            if (p.mouseoverRowCssClass) {
                for (var i in g.rows) {
                    ...
                    //---- @2015-4-12 By Yuanxianfu. -- 毙掉鼠标移动改变样式 ----
        ...
        _applyEditor: function (obj) {
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                g.trigger('cellEditing', [g.editor]); //wangfeng 2013-12-10 控件绘制好后触发
                g.unbind('sysEndEdit');
        ...
        _setEvent: function () {
            ...
            $(document).bind("keydown.grid", function (e) {
                ...
                if (e.shiftKey) g.shiftKey = true; //----wangfeng 20140414
                if (e.keyCode == 70) {
                    g.ctrlKey = false;
                    g.shiftKey = false; //----wangfeng 20140414
                }
            });
            $(document).bind("keyup.grid", function (e) {
                delete g.ctrlKey;
                delete g.shiftKey; //----wangfeng 20140414
            });
            //表体 - 滚动联动事件 
        ...
        select: function (rowParm) {
            ...
            if (!g.enabledCheckbox() && !g.ctrlKey && !g.shiftKey) //单选
            {
            ...
            //----wangfeng 20140414 shiftKey
            if (!g.enabledCheckbox() && g.shiftKey && g.selected.length > 0) //按住shift多选
            {
                //---先查找最小 选中项目，再找到最大选择项目 ---
                var MinIndex = -1;
                var MaxIndex = -1;
                var NowSelIndex = rowdata['__index'];
                for (var i in g.selected) {
                    var o = g.selected[i];
                    if (MaxIndex==-1) {
                        MaxIndex = o['__index'];
                    ...
                    //------循环找出最大序号与最小序号-------
                    if (o['__index'] > MaxIndex) {
                        MaxIndex = o['__index'];
                    }
                    if (o['__index'] < MinIndex) {
                        MinIndex = o['__index'];
                    }
                }
                if (NowSelIndex > MaxIndex) {
                    MaxIndex = NowSelIndex;
                }
                if (NowSelIndex < MinIndex) {
                    MinIndex = NowSelIndex;
                }
                ...
                if (MinIndex > -1 && MaxIndex > -1) {
                    ...
                    for (; MinIndex <= MaxIndex; MinIndex++) {
                        var rowdataNow = g.data.Rows[MinIndex];
                        var rowidNow = rowdataNow['__id'];
                        var rowobjNow = g.getRowObj(rowidNow);
                        var rowobj1Now = g.getRowObj(rowidNow,true);

                        if (rowobjNow) $(rowobjNow).addClass("l-selected");
                        if (rowobj1Now) $(rowobj1Now).addClass("l-selected");
                        // g.selected[g.selected.length] = rowdataNow;
                        g.selected[iii] = rowdataNow;
                        iii = iii + 1;
                    }
                }
            }

            if (iii == 0) //----没有进行多行shiftKey选择时----
            {
                if (rowobj) $(rowobj).addClass("l-selected");
                if (rowobj1) $(rowobj1).addClass("l-selected");
                g.selected[g.selected.length] = rowdata; //在ctrlKey时会加入选中行
