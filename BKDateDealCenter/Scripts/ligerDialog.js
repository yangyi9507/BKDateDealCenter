...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        closeCallFun: null          //关闭弹窗时的回调函数 --zf 20161117
    };
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            if (!p.fixedType)
            {
                ...
                var height = p.height || g.dialog.height();//wangfeng 修改特殊情况下弹出位置不正确

                if (p.slide == true) p.slide = 'fast';
                if (p.left != null) left = p.left;
                else p.left = left = 0.5 * ($(window).width() - width);
                if (p.top != null) top = p.top;
                else p.top = top = 0.5 * ($(window).height() - height) + $(window).scrollTop() - 10;
